...
class Assistant(Agent):
    ...
    async def on_tool_result(self, tool_name, user_input, result):
        ...
        print(f"🔧 Tool: {tool_name} | Result: {str(result)[:100]}")
        try:
    ...
    async def on_reply(self, user_input, jarvis_reply):
        """Called when agent generates speech response"""
...
async def entrypoint(ctx: agents.JobContext):
    ...
    # Create session with Google Realtime (handles both listening & speaking)
    session = AgentSession(
        llm=google.beta.realtime.RealtimeModel(
            voice="Charon",  # Built-in TTS voice
            model="gemini-2.0-flash-exp"  # Use latest model
        )
    )
    
    print("🎤 Initializing audio with noise cancellation...")
    await session.start(
        room=ctx.room,
        agent=Assistant(),
    ...
    await ctx.connect()
    print("✅ JARVIS is LIVE and listening...")
    ...
    async def safe_generate_reply(sess, instructions, retries: int = 3, base_wait: float = 1.0) -> bool:
        ...
        for attempt in range(1, retries + 1):
            try:
                ...
                entry = f"[{ts}] Attempt {attempt} failed: {e}\n{tb}\n"
                ...
                print(f"⚠️ Greeting attempt {attempt}/{retries} failed: {e}")
                if attempt < retries:
    ...
    if not ok:
        print("❌ Failed to generate greeting. Check jarvis_data/realtime_errors.log")


if __name__ == "__main__":
    # Auto-start GUI (optional visual interface)
    def start_gui_if_needed():
        try:
            ...
            if os.path.exists(venv_pythonw):
                ...
                pythonw = sibling_pythonw if os.path.exists(sibling_pythonw) else sys.executable
            ...
            if os.path.exists(gui_script):
                try:
                    ...
                    with builtins.open(lock_path, 'w') as lk:
                        lk.write(str(os.getpid()))
                    print("🖥️ GUI started")
                except Exception as e:
                    print(f"⚠️ GUI start failed: {e}")
        except Exception:
            pass
    ...
    print("\n" + "="*50)
    print("🤖 JARVIS AGENT - LiveKit Realtime Mode")
    print("="*50 + "\n")
